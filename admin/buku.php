    ...
    <div class="d-flex justify-content-between align-items-center m-5 mt-2">
      <form action="" method="post" class="d-flex">
        <div class="layout-card-custom">
          <button class="btn btn-info" type="submit" name="kategori" value="">Semua</button>
          <?php foreach ($kategori as $item): ?>
            <button type="submit" name="kategori" value="<?= $item['kategori']; ?>" class="btn btn-outline-info"><?= ucfirst($item['kategori']); ?></button>
          <?php endforeach; ?>
        </div>
      </form>
      ...
      <?php foreach ($buku as $item) : ?>
        <div class="card" style="width: 15rem;">
          ...
          <div class="card-footer bg-light d-flex justify-content-between">
            <a class="btn btn-sm btn-success" href="bukuUpdate.php?idReview=<?= $item["id_buku"]; ?>" id="review">Edit</a>
            <a class="btn btn-sm btn-danger" href="bukuDelete.php?id=<?= $item["id_buku"]; ?>" onclick="return confirm('Yakin ingin menghapus data buku ? ');">Delete</a>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
